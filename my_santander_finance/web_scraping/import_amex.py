"""
importa el .xls de la tarjeta de amex de santander Argentina a una base de datos sqlite
"""
import csv
import os
import shutil
...
def xls_to_csv(src_dir: str, src_ext: str, dst_dir: str, dst_ext: str):
    ...
    files = get_list_files(dir=src_dir, ext=src_ext, prefix="amex")
...
def csv_to_sqlite(cvs_filepath: str, sqlite_filepath: str):
    ...
    for row in reader:
        ...
        try:
            curs.execute("INSERT INTO amex VALUES (?, ?, ?, ?, ?, ?, ?, ?);", to_db)
            count = count + 1
        except:  # noqa: E722
            skip = skip + 1
...
def import_csv_to_sqlite():
    ...
    files = get_list_files(dir=settings.CVS_TEMP_DIR, ext=".csv", prefix="amex")
...
if __name__ == "__main__":
    xls_to_csv(
        src_dir=settings.DOWNLOAD_AMEX_DIR,
        src_ext=".xls",
